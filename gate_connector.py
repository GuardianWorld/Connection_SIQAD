...
def connect_2_gates_left(gate1, gate2, wire=None, wires=0):
    ...
    m_pos = leftmost_perturber.latcoord['m']
    n_pos = leftmost_perturber.latcoord['n']
    for i in range(wires):
        for j in range(2):
            m_pos -= 1
            n_pos -= 2
    ...
    for perturber in gate1.input_perturbers:
        if(perturber != leftmost_perturber):
...
def connect_2_gates_right(gate1, gate2, wire=None, wires=0):
    input_perturbers = []
    
    # Connect the output of gate1 to the input of gate2
    # Finds the rightmost perturber of gate 1 and connects gate 2 to it.
    
    rightmost_perturber = sqd_manipulator.find_most_right_dot(gate1.input_perturbers)
    n = rightmost_perturber.latcoord['n'] + 2
    m = rightmost_perturber.latcoord['m'] - 1
    ...
    #step 1.4: add the wire dots.
    #it always goes 1 up, 2 to the side.
    m_pos = rightmost_perturber.latcoord['m']
    n_pos = rightmost_perturber.latcoord['n']
    for i in range(wires):
        for j in range(2):
            m_pos -= 1
            n_pos += 2
    ...
    #step 2: get a list of the input perturbers of both gates (remaining).
    for perturber in gate1.input_perturbers:
        if(perturber != rightmost_perturber):
            input_perturbers.append(perturber)
